...
$scope.idleTimer=$timeout(function(){$scope.updateBounds();$scope.isShortLoading=!$scope.isLoading;$scope.loadContent(!0);window.markerClusterer=new MarkerClusterer($scope.crMap,[],{maxZoom:15,minimumClusterSize:2,imagePath:'./img/cluster/m'})},750)})};GoogleMaps.addCallback($scope.initMap);Loader.show($translate.instant('Loading...','places'))});angular.module('starter').controller('PlacesViewController',function($filter,$scope,$rootScope,$state,$stateParams,$translate,$location,Places,SocialSharing,Url,Dialog,Loader,Customer){angular.extend($scope,{is_loading:!0,value_id:$stateParams.value_id,social_sharing_active:!1,use_pull_to_refresh:!0,pull_to_refresh:!1,card_design:!1,notes_are_enabled:Places.settings.notesAreEnabled,localData:{notes:[]},form:{note:""}});$scope.blankImage="./features/places/assets/templates/l1/img/blank-700-440.png";Places.setValueId($stateParams.value_id);$scope.loadContent=function(){Places.getPlace($stateParams.page_id).then(function(data){$scope.social_sharing_active=(data.social_sharing_active&&$rootScope.isNativeApp);$scope.blocks=data.blocks;$scope.blockChunks=$filter('chunk')(angular.copy($scope.blocks),Math.ceil($scope.blocks.length/2));$scope.place=data.page;$scope.page_title=data.page_title}).then(function(){$scope.is_loading=!1});$scope.loadNotes($stateParams.page_id)};$scope.share=function(){var file;var address="";var link=undefined;angular.forEach($scope.blocks,function(block){if(block.gallery){if(block.gallery.length>0&&file===null){file=block.gallery[0].url}}
if(block.type==="address"){address=block.address;if(block.website!==""&&block.show_website){link=block.website}}});var message="Check this place!\n"+$scope.place.title+"\n"+address;SocialSharing.share(undefined,message,undefined,link,file)};$scope.isLoggedIn=function(){return Customer.isLoggedIn()};$scope.login=function(){return Customer.loginModal(undefined,$scope.loadContent,$scope.loadContent,$scope.loadContent)};$scope.onShowMap=function(block){if($rootScope.isNotAvailableOffline()){return}
...
params.title=block.label;params.value_id=$scope.value_id;$location.path(Url.get('map/mobile_view/index',params))};$scope.sendNote=function(){if($scope.form.note.length<10){return Dialog.alert("Error","Note must be at least 10 characters.","OK","places")}
Loader.show($translate.instant("Saving note...","places"));Places.createNote($stateParams.page_id,$scope.form.note).then(function(success){Dialog.alert("","Note is saved!","OK","places");$scope.form.note="";$scope.loadNotes($stateParams.page_id)},function(error){Dialog.alert("Error",error.message,"OK","places")}).then(function(){Loader.hide()})};$scope.deleteNote=function(noteId){Dialog.confirm('Confirmation','You are about to delete this note!',['YES','NO'],-1,'places').then(function(value){if(!value){return}
Loader.show();Places.deleteNote($stateParams.page_id,noteId).then(function(success){Dialog.alert("","Note is removed!","OK","places");$scope.loadNotes($stateParams.page_id)},function(error){Dialog.alert("Error",error.message,"OK","places")}).then(function(){Loader.hide()})})};$scope.loadNotes=function(placeId){if(!Places.settings.notesAreEnabled){return}
Places.findNotes(placeId).then(function(success){$scope.localData.notes=success.notes})};$scope.loadContent()});angular.module('starter').factory('Places',function($pwaRequest){var factory={value_id:null,collection:[],mapCollection:[],extendedOptions:{},settings:[]};factory.setValueId=function(value_id){factory.value_id=value_id};factory.setExtendedOptions=function(options){factory.extendedOptions=options};factory.setMapCollection=function(collection){factory.mapCollection=collection};factory.preFetch=function(){factory.findAll()};factory.findAll=function(filters,refresh){if(!this.value_id){return $pwaRequest.reject('[Factory::Places.findAll] missing value_id')}
...
return $pwaRequest.get('places/mobile_list/find-one',{urlParams:{value_id:this.value_id,place_id:placeId}})};factory.createNote=function(placeId,note){if(!this.value_id){return $pwaRequest.reject('[Factory::Places.createNote] missing value_id')}
return $pwaRequest.post('places/mobile_view/save-note',angular.extend({urlParams:{value_id:this.value_id},data:{place_id:placeId,note:note}},factory.extendedOptions))};factory.deleteNote=function(placeId,noteId){if(!this.value_id){return $pwaRequest.reject('[Factory::Places.deleteNote] missing value_id')}
return $pwaRequest.post('places/mobile_view/delete-note',{urlParams:{value_id:this.value_id},data:{place_id:placeId,note_id:noteId}})};factory.findNotes=function(placeId){if(!this.value_id){return $pwaRequest.reject('[Factory::Places.findNotes] missing value_id')}
return $pwaRequest.post('places/mobile_list/find-notes',{urlParams:{value_id:this.value_id},data:{place_id:placeId},refresh:!0})};factory.getPlace=function(placeId){if(!this.value_id){return $pwaRequest.reject('[Factory::Places.getPlace] missing value_id')}
