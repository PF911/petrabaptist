...
if(refresh===!0){$scope.isLoading=!1}})};$rootScope.$on('fanwall.refresh',function(){if($scope.currentTab==='post'){$scope.loadContent(!0)}});$scope.loadContent($scope.collection.length===0)});angular.module('starter').controller('FanwallProfileController',function($scope,$rootScope,$stateParams,$timeout,Customer,FanwallUtils,FanwallPost,Lightbox){angular.extend($scope,{isLoading:!0,collection:[],customer:Customer.customer,hasMore:!1});$scope.customerFullname=function(){return $scope.customer.firstname+' '+$scope.customer.lastname};$scope.profileCallback=function(){$scope.loadContent(!0,!1);$timeout(function(){$scope.customer=Customer.customer})};$scope.customerImagePath=function(){if($scope.customer.image&&$scope.customer.image.length>0){return IMAGE_URL+'images/customer'+$scope.customer.image}
...
var deleteText=$translate.instant('Trash post','fanwall');if($scope.post.status==='deleted'){deleteText=$translate.instant('Delete permanently','fanwall')}
$scope.popoverItems.push({label:deleteText,icon:'icon ion-android-delete',click:function(){$scope.closeActions().then(function(){$scope.deletePost()})}})}else{$scope.popoverItems.push({label:$translate.instant('Report post','fanwall'),icon:'icon ion-flag',click:function(){$scope.closeActions().then(function(){$scope.flagPost()})}});if($scope.post.history.length>0){$scope.popoverItems.push(viewHistory)}
